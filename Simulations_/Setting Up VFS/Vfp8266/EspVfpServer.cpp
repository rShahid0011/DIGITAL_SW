...
void (*Server::loginHandler)();
void (*Server::logoutHandler)();
...
void Server::authorize(bool flag)
 { if (flag != authorized)
    { authorized = flag;
      if (flag && loginHandler != NULL)
         loginHandler();
      if (!flag && logoutHandler != NULL)
         logoutHandler();
      set("$", F("authorized")).arg(flag).end();           
    }
 }
...
void Server::poll () 
 { while (process.available() > 0) 
    { String command = process.readStringUntil(':');
      ...
      IPAddress addr;         
      if (command == F("RELOAD"))
       { // Server passes client IP address as a parameter
         // If it's different then we reset the authorization state.
         if (addr.fromString(process.readStringUntil('*')))
            if (addr != ipaddress) 
             { ipaddress = addr;
               authorize(false);
             }
      ...
      else if (command == F("TIMEOUT"))
       { // Client has gone away
         ipaddress = (unsigned long)0;
         authorize(false);
       }
